...
from flask import Flask, render_template, request, jsonify, send_file, make_response, session, flash, redirect, url_for
import pandas as pd
import json
import os
import numpy as np
...
from utils.user_auth import user_auth, login_required, admin_required
import threading
import time
# 配置日志
...
# 配置会话密钥
app.config['SECRET_KEY'] = 'hunan-agriculture-analysis-system-2025'
...
def login():
    ...
    if request.method == 'POST':
        ...
        success, message = user_auth.authenticate_user(username, password)
...
def logout():
    ...
    session.clear()
    flash('已成功登出，再见' + username + '！', 'success')
    return redirect(url_for('login'))
...
def change_password():
    ...
    if request.method == 'POST':
        ...
        success, message = user_auth.change_password(username, old_password, new_password)
